 ...
 */
export function matchQueryCondition<T extends { _id: string; [key: string]: any }>(
  data: T,
 ...
 */
export function query<T extends QueryItem>(condition?: QueryCondition<T> | null) {
 ...
 */
export function parseQueryCondition(condition?: QueryCondition<any> | null): Array<(item: any) => boolean> {
  ...
  for (let keyOrPropName in condition) {
    ...
    if (keyOrPropName === "$or" || keyOrPropName === "$nor" || keyOrPropName === "$and") {
      const filterConditions = conditionValue as QueryCondition<any>[]
...
export type QueryCondition<TSchema extends QueryItem> = Partial<{
